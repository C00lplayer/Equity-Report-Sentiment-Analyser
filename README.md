...
- `using/dashboard.py` defines a Streamlit dashboard interface for selecting equity reports by ticker, year, and source.
- `using/text_extracter.py` retrieves report links from Supabase and extracts article text from supported web sources.
- It also extracts and cleans GPFS PDF text, avoiding table content when possible.
- `using/sentence_model.py` handles sentence splitting, chunking, and FinBERT sentiment scoring.
...
- `using/dashboard.py` – dashboard logic and analysis flow.
- `using/text_extracter.py` – text retrieval from web articles and GPFS PDFs.
- `using/sentence_model.py` – FinBERT sentiment pipeline and aggregation.
- `using/requirements.txt` – project dependencies.
